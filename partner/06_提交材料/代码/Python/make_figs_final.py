...
M = A['main']; Q = M['q_star']; n, m2 = D.n, D.m2
...
R30 = AC.run_q3(q=Q, adjust=(), use_adjbuf=True)     # 问题三: 不调整
R312 = AC.run_q3(q=Q, adjust=(12,), use_adjbuf=True)  # 问题三: 12:00 调整(采用)
R42 = AC.run_q2(q=Q, pmat=PR)                        # 问题四之二(电价已知, 采用)
R42b = AC.run_q2(D.Lb, D.Pb, q=Q, pmat=PR)           # 问题四之二 原依据对照
R4312 = AC.run_q3(q=Q, pmat=PR, adjust=(12,), use_adjbuf=True)   # 问题四之三: 12:00 调整(采用)
print(f'  采用 Q2 {R2["total"]:,.0f} | 原依据 {R2b["total"]:,.0f} | Q3 +12 {R312["total"]:,.0f} '
      f'| Q4-2 {R42["total"]:,.0f} | Q4-3 +12 {R4312["total"]:,.0f}')
...
labs = ['Q4-2\n升级预测\n(采用)', 'Q4-2\n原依据', 'Q4-2\n用预测电价', 'Q4-2\n无储能',
        'Q4-3\n12:00调整\n(采用)', 'Q4-3\n全滚动']
v = [M['q42']['total'], R42b['total'], M['q42']['pred_price'],
     A['q4_nostorage'], M['q43']['total'], o43['全滚动']['total']]
ax[0].bar(labs, np.array(v) / 1e4, color=[GREEN, GRAY, BLUE, ORANGE, PURPLE, GRAY])
...
d43 = (M['q42']['total'] - M['q43']['total']) / 1e4
ax[1].bar(['Q4-2 不调整', 'Q4-3 12:00 调整\n(采用)', 'Q4-3 全滚动'],
          [M['q42']['total'] / 1e4, M['q43']['total'] / 1e4, o43['全滚动']['total'] / 1e4],
          color=[GREEN, PURPLE, GRAY])
for i, x in enumerate([M['q42']['total'], M['q43']['total'], o43['全滚动']['total']]):
    ax[1].text(i, x / 1e4 + 2, f'{x/1e4:.1f}', ha='center', fontsize=9)
ax[1].set_ylabel('全年费用 (万元)')
ax[1].set_title(f'波动电价下 12:00 调整的价值 {d43:+.1f} 万元')
ax[1].grid(alpha=0.3, axis='y'); ax[1].tick_params(axis='x', labelsize=8)
p4 = PR[sel]
ax[2].plot(t_ax, p4, color=ORANGE, lw=1.3)
ax2 = ax[2].twinx()
ax2.plot(t_ax, R4312['Pf'][sel] * 6, color=BLUE, lw=1.2, label='12:00 调整后购电')
