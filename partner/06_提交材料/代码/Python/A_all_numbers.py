...
Q = A['q_star']
out = dict(main=A, load_weights=AC.LOAD_INFO)
...
for at, nm in [((), '仅0:00'), ((6,), '+6:00'), ((12,), '+12:00'), ((18,), '+18:00'),
    ...
    R = AC.run_q3(q=Q, adjust=at, use_adjbuf=True)
...
sens['price_pred'] = float(AC.run_q2(q=Q, pmat=P_RT)['total'])
...
out['q4_nostorage_predprice'] = float(AC.nostorage(q=Q, pmat=P_RT))
out['q4_bound_predprice'] = float(AC.clairvoyant(pmat=P_RT))
R42 = AC.run_q2(q=Q, pmat=PR)          # 问题四之二 采用方案(电价已知)
R42p = AC.run_q2(q=Q, pmat=P_RT)       # 问题四之二 计划用预测电价
