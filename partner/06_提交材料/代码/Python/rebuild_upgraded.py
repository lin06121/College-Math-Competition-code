...
def run_q3(Lh, PVh, q, pmat=None, adjust=(6, 12, 18), upto=None):
    ...
    for i in range(upto):
        p = pm[i]
        v = (0.9 * price.mean() if pmat is None else 0.9 * pm[min(i + 1, n - 1)].mean()) if i < n - 1 else 0.0
        b = buffer_of(Lh, PVh, q, i); Lbar = Lh[i] + b
...
ADJ = (12,)          # ★ 正式采用: 只在 12:00 引入预报调整(6/18 时调整会因偏差费超过收益而变差)
R2 = run_q2(L_UP, PV_UP, q=Q_STAR)
print(f'问题二: 计划 {R2["plan"]:,.0f} + 紧急 {R2["em"]:,.0f} = {R2["total"]:,.0f} 元')
R3 = run_q3(L_UP, PV_UP, q=Q_STAR, adjust=ADJ)
print(f'问题三(仅12:00调整): 计划 {R3["plan"]:,.0f} + 调整 {R3["dev"]:,.0f} + 紧急 {R3["em"]:,.0f} = {R3["total"]:,.0f} 元')
R42 = run_q2(L_UP, PV_UP, q=Q_STAR, pmat=PR)
print(f'问题四之二: 计划 {R42["plan"]:,.0f} + 紧急 {R42["em"]:,.0f} = {R42["total"]:,.0f} 元')
R43 = run_q3(L_UP, PV_UP, q=Q_STAR, pmat=PR, adjust=ADJ)
print(f'问题四之三(仅12:00调整): 计划 {R43["plan"]:,.0f} + 调整 {R43["dev"]:,.0f} + 紧急 {R43["em"]:,.0f} = {R43["total"]:,.0f} 元')
R3z = run_q3(L_UP, PV_UP, q=Q_STAR, adjust=())
print(f'问题三(仅0:00, 对照): {R3z["total"]:,.0f} 元')
R3f = run_q3(L_UP, PV_UP, q=Q_STAR, adjust=(6, 12, 18))
print(f'问题三(全滚动, 对照): {R3f["total"]:,.0f} 元')
R43z = run_q3(L_UP, PV_UP, q=Q_STAR, pmat=PR, adjust=())
R43f = run_q3(L_UP, PV_UP, q=Q_STAR, pmat=PR, adjust=(6, 12, 18))
...
num = dict(q_star=Q_STAR, jan_grid=jan, adopt_adjust='仅12:00',
