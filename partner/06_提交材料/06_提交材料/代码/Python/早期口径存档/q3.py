...
def run_rolling(adjust_times=(6, 12, 18)):
    ...
    for i in range(n):
        ...
        if 6 in adjust_times:
            h0 = H6
            r = solve_day(price[h0:], L_prev[i, h0:], fca_m[6][i, h0:], E0[h0], soc_end=None, v=v, base_P=P0[h0:])
            P[h0:], C[h0:], D[h0:] = r['P'], r['C'], r['D']
            E = np.concatenate([E[:h0 + 1], r['E'][1:]])
            S[h0:], X[h0:] = r['S'], r['X']
        if 12 in adjust_times:
            h0 = H12
            r = solve_day(price[h0:], L_prev[i, h0:], fca_m[12][i, h0:], E[h0], soc_end=None, v=v, base_P=P0[h0:])
            P[h0:], C[h0:], D[h0:] = r['P'], r['C'], r['D']
            E = np.concatenate([E[:h0 + 1], r['E'][1:]])
            S[h0:], X[h0:] = r['S'], r['X']
        if 18 in adjust_times:
            h0 = H18
            r = solve_day(price[h0:], L_prev[i, h0:], fca_m[18][i, h0:], E[h0], soc_end=None, v=v, base_P=P0[h0:])
            P[h0:], C[h0:], D[h0:] = r['P'], r['C'], r['D']
            E = np.concatenate([E[:h0 + 1], r['E'][1:]])
            S[h0:], X[h0:] = r['S'], r['X']
